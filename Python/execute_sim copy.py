...
def runSim(cancer_type, weight_type, communication_type, pats="all", N=10000, itNo=100, av=15, norm=False, folder_dir="Input_data_RaCInG"):
    """
    Provides an example funciton that reads input information, generates graphs,
    ...
    # Read input information
    lig, rec, Dcell, Dconn, cells, _, _, _ = generateInput(
        weight_type, cancer_type, folder_dir=folder_dir)
    ...
    if norm:
        ...
        if communication_type == "D":
            ...
            filename = "{}_{}_{}_norm.out".format(
                cancer_type, communication_type, av)
    else:
        if communication_type == "D":
            filename = "{}_D.out".format(cancer_type)
        else:
            filename = "{}_{}_{}.out".format(
                cancer_type, communication_type, av)
...
if __name__ == "__main__":
    ...
    runSim(cancer_type=args.cancer_type, weight_type=args.weight_type, communication_type=args.communication_type, pats=args.patient_id,
