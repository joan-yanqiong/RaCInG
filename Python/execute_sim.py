...
def runSim(cancer_type, weight_type, communication_type, pat=0, N=10000, itNo=100, av=15, norm=False, output_dir="", input_dir="Input_data_RaCInG"):
    """
    Provides an example funciton that reads input information, generates graphs,
...
def combinePatients(input_dir, cancer_type, communication_type, av, norm, output_dir):
    ...
    is_patient_file = np.array([filename.find("meta") == -1 for filename in files])
