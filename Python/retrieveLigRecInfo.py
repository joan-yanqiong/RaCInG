        ...
        for i in range(Dconn.shape[0]):
            for j in range(Dconn.shape[1]):
                ...
                if weightlig != 0 and weightrec != 0:
                    scoring[i, j, patient] = Dconn[i, j, patient] * \
                        liglist[c1, i] * reclist[c2, j] / \
                        (weightlig * weightrec)
                    factors[i, j] = Dconn[i, j, patient] * liglist[c1,
                                                                   i] * reclist[c2, j] / (weightlig * weightrec)
            ...
            for j in range(Dconn.shape[1]):
                ...
                if weightlig != 0 and weightrec != 0:
                    scoring[i, j, patient] = Dconn[i, j, patient] * \
                        liglist[c1, i] * reclist[c2, j] / \
                        (weightlig * weightrec)
                    scoring2[i, j, patient] = Dconn[i, j, patient] * \
                        liglist[c2, i] * reclist[c1, j] / \
                        (weightlig * weightrec)
                    factors[i, j] = Dconn[i, j, patient] * liglist[c1,
                                                                   i] * reclist[c2, j] / (weightlig * weightrec)
                    factors2[i, j] = Dconn[i, j, patient] * liglist[c2,
                                                                    i] * reclist[c1, j] / (weightlig * weightrec)
        normlist[patient] = np.sum(factors)
...
def findLigRecScoring(liglist, reclist, Dcell, Dconn, cellnames, c1, c2, c3):
    ...
    weights : np.array() with float entries;
        The baysian scoring of all ligand-receptor pairs given the cell type.
    """
    a = calculateBaysianInteractionScoreDirected(
        liglist, reclist, Dcell, Dconn, cellnames, c1, c2)
    b = calculateBaysianInteractionScoreDirected(
        liglist, reclist, Dcell, Dconn, cellnames, c2, c3)
    c = calculateBaysianInteractionScoreDirected(
        liglist, reclist, Dcell, Dconn, cellnames, c1, c3)
...
def createPicture(weighttype, cancer, c1, c2, c3, g1, g2, feature, thresh, folder, triangle=True):
    ...
    liglist, reclist, Dcell, Dconn, cellnames, ligands, receptors, _ = generateInput(
        weighttype, cancer, folder=folder)
    _, _, patients = createCellTypeDistr(cellnames, os.path.join(
        current_path, folder, r"{}_TMEmod_cell_fractions.csv".format(cancer)))
    ...
    # except:
    #     subtypes = Metadata_csv_read_in(
    #         "metadata_{}.csv".format(cancer), feature)
    ...
    # # Find probabilities
    if triangle:
        weights = findLigRecScoring(
            liglist, reclist, Dcell, Dconn, cellnames, c1, c2, c3)
    else:
        weights = calculateBaysianInteractionScoreUndirected(
            liglist, reclist, Dcell, Dconn, cellnames, c1, c2)

    # Compute average scores over group
    scoresA = np.average(
        weights[:, :, np.array(subtypes)[:NoPat] == g1], axis=2)
    scoresB = np.average(
        weights[:, :, np.array(subtypes)[:NoPat] == g2], axis=2)
    ...
    if triangle:
        plt.title("Interaction probabilities for Tr_{}_{}_{} feature in {}".format(
            c1, c2, c3, cancer))
    else:
...
if __name__ == "__main__":
    ...
    createPicture("min", "kim", "B", "CD8+ T", "Grr", "R", "NR",
                  "Response", 20, "Example input", triangle=False)
